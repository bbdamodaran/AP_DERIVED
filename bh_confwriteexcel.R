bh_confwriteexcel<-function(AccMatrix, fname,option)
{
library(xlsx)
  
#classifier<-list("PerTurbo","SVM","DecTree","RandForst")
classifier<-list("PerTurbo","SVM","DecTree","RandForst","NBC","MLR")

nfolds<-nrow(AccMatrix$OA)
sheetnames<-paste0(classifier[option],"Fold",seq(from=1,to=nfolds, by=1))

# # create workbooks and styles
# setwd("D:/PostDocWork/Denmark/AttributeProfiles/ClassificationResults/NDVI+NDVIAP_500")
...
for (i in 1:nfolds)
{
  sheet<-createSheet(wp, sheetName=sheetnames[i])
   Title<-"ISPRS7 OrgImg+NDVI+DSM+OrgImgAP Classification"
  ...
  #
  addDataFrame(AccMatrix$conf[i], sheet, startRow=10, startColumn=4,row.names = TRUE, col.names = TRUE)
  ...
  #
  addDataFrame(AccMatrix$PA[i], sheet, startRow=23, startColumn=4,row.names = TRUE, col.names = TRUE)
  ...
  #
  addDataFrame(AccMatrix$UA[i], sheet, startRow=23, startColumn=17,row.names = TRUE, col.names = TRUE)
