...
for (outer in 1:length(NPer))
{
  ...
  TrainTestSampleIndex<-list()
  #
  DT<-list();DTDV<-list();RF<-list();RFDV<-list();NB<-list();NBDV<-list();MLR<-list();MLRDV<-list();
  DTClassifiedlabel<-list();DTTrainingParameter<-list();RFClassifiedlabel<-list();RFTrainingParameter<-list();NBClassifiedlabel<-list();NBTrainingParameter<-list();
  MLRClassifiedlabel<-list();MLRTrainingParameter<-list();
  DT$OA<-numeric(0);RF$OA<-numeric(0);NB$OA<-numeric(0);MLR$OA<-numeric(0);
...
#     testlabel<-orglabel[sss$test_index]
    ...
    DAcc<-bh_confusionmat(testlabel,DTPrediction$classifiedlabel[testlocation_index],option="CI")
    ...
    DT$UA[[loop]]<-DAcc$UA
    DT$PA[[loop]]<-DAcc$PA
    rm(DTmodel, DTPrediction)
    cat('DT Over loop', loop)
    
    source('bh_ENVIWrite.R')
    source('bh_EnviHeaderWrite.R')
    setwd("/share/home/damodara/DenmarkWork/RCodes/Results/ISPRS_7/Original Features/OriginalImage_500samples")
    fname<-paste("ISPRS_7_DT_CLassifiedImage_", as.character(NPer),"fold_", as.character(loop), sep="")
    Img<-reshape(as.matrix(PerTurboPrediction$classifiedlabel), sz[1],sz[2])
    bh_ENVIWrite(Img,fname)
    bh_EnviHeaderWrite(Img,fname)
    setwd(old_dir)
    rm(PerTurboPrediction)
    rm(Img)
    ...
    #confusion matrix
    RAcc<-bh_confusionmat(testlabel,RFPrediction$classifiedlabel[testlocation_index],option="CI")
    ...
    rm(RFmodel, RFPrediction)
    cat('RF Over loop', loop)
    
    source('bh_ENVIWrite.R')
    source('bh_EnviHeaderWrite.R')
    setwd("/share/home/damodara/DenmarkWork/RCodes/Results/ISPRS_7/Original Features/OriginalImage_500samples")
    fname<-paste("ISPRS_7_RF_CLassifiedImage_", as.character(NPer),"fold_", as.character(loop), sep="")
    Img<-reshape(as.matrix(PerTurboPrediction$classifiedlabel), sz[1],sz[2])
    bh_ENVIWrite(Img,fname)
    bh_EnviHeaderWrite(Img,fname)
    setwd(old_dir)
    ...
    #confusion matrix
    NAcc<-bh_confusionmat(testlabel,NBPrediction$classifiedlabel[testlocation_index],option="CI")
    ...
    rm(NBmodel, NBPrediction)
    cat('NB Over loop', loop)
    
    source('bh_ENVIWrite.R')
    source('bh_EnviHeaderWrite.R')
    setwd("/share/home/damodara/DenmarkWork/RCodes/Results/ISPRS_7/Original Features/OriginalImage_500samples")
    fname<-paste("ISPRS_7_NBC_CLassifiedImage_", as.character(NPer),"fold_", as.character(loop), sep="")
    Img<-reshape(as.matrix(PerTurboPrediction$classifiedlabel), sz[1],sz[2])
    bh_ENVIWrite(Img,fname)
    bh_EnviHeaderWrite(Img,fname)
    setwd(old_dir)
    ...
    stopCluster(cl)
    source('bh_list2matix.R')
    MLRPrediction<-bh_list2matix(MLRPer)
    rm(MLRPer)
    ...
    #confusion matrix
    MAcc<-bh_confusionmat(testlabel,MLRPrediction$classifiedlabel[testlocation_index],option="CI")
    ...
    rm(MLRmodel, MLRPrediction)
    cat('MLR Over loop', loop)
    
    source('bh_ENVIWrite.R')
    source('bh_EnviHeaderWrite.R')
    setwd("/share/home/damodara/DenmarkWork/RCodes/Results/ISPRS_7/Original Features/OriginalImage_500samples")
    fname<-paste("ISPRS_7_MLR_CLassifiedImage_", as.character(NPer),"fold_", as.character(loop), sep="")
    Img<-reshape(as.matrix(PerTurboPrediction$classifiedlabel), sz[1],sz[2])
    bh_ENVIWrite(Img,fname)
    bh_EnviHeaderWrite(Img,fname)
    setwd(old_dir)
  ...
  save(DTDV, RFDV, NBDV,MLRDV, classifiers, file=paste("ISPRS_Classifiers_DV_OrgImg_",
                                                       as.character(NPer[outer]), sep=""))
  # Classifiedlabels
  save(DTClassifiedlabel, RFClassifiedlabel, NBClassifiedlabel, MLRClassifiedlabel,classifiers,
       file=paste("ISPRS_Classifiers_OrgImg_Classifiedlabel_", as.character(NPer[outer]), sep=""))
  setwd(old_dir)
  
  # save Sample Location Index
  #save(TrainTestSampleIndex, NPer, file=paste("ISPRS_7_NDVIAP_TRTestSampleLocationIndex_",as.character(NPer[outer]),sep=""))
  
  rm(DTDV, RFDV, NBDV, MLRDV,DTClassifiedlabel, RFClassifiedlabel, NBClassifiedlabel, MLRClassifiedlabel)
  ...
  # Decision Tree
  fname=paste("ISPRS7_OrgImg_DT_tr",trp,"_test",testp,".xlsx", sep="")
  #fname="ISPRS7_DT_10tr_90test.xlsx"
  option<-3
  setwd(old_dir)
  bh_confwriteexcel_cluster(DT, fname,option)
  # Random Forest
  fname=paste("ISPRS7_OrgImg_RF_tr",trp,"_test",testp,".xlsx", sep="")
  #fname="ISPRS7_RF_10tr_90test.xlsx"
  option<-4
  setwd(old_dir)
  bh_confwriteexcel_cluster(RF, fname,option)
  # Naive Bayes Forest
  fname=paste("ISPRS7_OrgImg_NBC_tr",trp,"_test",testp,".xlsx", sep="")
  #fname="ISPRS7_RF_10tr_90test.xlsx"
  option<-5
  setwd(old_dir)
  bh_confwriteexcel_cluster(NB, fname,option)
  # multinomial logistic reg classifier
  fname=paste("ISPRS7_OrgImg_MLRC_tr",trp,"_test",testp,".xlsx", sep="")
  #fname="ISPRS7_RF_10tr_90test.xlsx"
  option<-6
  setwd(old_dir)
  ...
  #
  setwd("/share/home/damodara/DenmarkWork/RCodes/Results/ISPRS_7/Original Features/OriginalImage_500samples")
  save(DT,file=paste("OrgImg_DT_Results_Percent_",as.character(NPer[outer]),sep=""))
  save(RF,file=paste("OrgImg_RF_Results_Percent_",as.character(NPer[outer]),sep=""))
  save(NB,file=paste("OrgImg_NBC_Results_Percent_",as.character(NPer[outer]),sep=""))
  save(MLR,file=paste("OrgImg_MLR_Results_Percent_",as.character(NPer[outer]),sep=""))
}

setwd("/share/home/damodara/DenmarkWork/RCodes/Results/ISPRS_7/Original Features/OriginalImage_500samples")
TrainingPercent<-NPer;
save(MeanAccuracy, StdAccuracy,TrainingPercent,classifiers,
     file=paste("OrgImg_ClassifiersClassificationAcc_", as.character(NPer[outer]), sep=""))